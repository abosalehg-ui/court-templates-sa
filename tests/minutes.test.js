...
test('composeMinutes: فقرة غياب المدعى عليه موضعها بعد رصد الدعوى ومصادقة المدعي', () => {
    ...
    // وتُفرد بسطر خالٍ عما قبلها وما بعدها
    assert.match(text, /صادق عليها\.\n\nوقد تبلـــــــــغ الـمـدعى عليه عــــــــــــــبر الوسائل الإلكترونية بمهمة التبليغ رقم \(456\)، \n\n/);
    assert.match(text, /من نظام المرافعات الشرعية\.\n\nوبسؤال المدعي عن بينته/);
    ...
    assert.match(M.composeMinutes(state), /هكذا قدَّم\.\n\nوقد تبلـــــــــغ الـمـدعى عليه/);
});
...
test('composeMinutes: مذكرة الدفاع الأولى مع الغياب تُرصد بلا عرض ولا مصادقة، ويُكلَّف المدعي بالبينة', () => {
    ...
    // ونصُّها مُفرد بسطر خالٍ عما قبله وما بعده، بلا تكرار للسطر الخالي بينه وبين فقرة الغياب
    assert.match(text, /ولا من يـمـثـــلــه\.\n\nثم فحصت الدائرة ملف القضية ووجد بأن المدعى عليه\n\nقد أودع مذكرة بدفاعه تتضمن النص التالي : \[ {3}\[ أنكر ما جاء في الدعوى \] {2}\] أ\. هـ\.\n\nولمَّا كانت/);
    assert.ok(!/\n{3}/.test(text));
...
test('composeMinutes: الجلسة المنظورة سابقًا تُبقي غياب المدعى عليه في فقرات الحضور', () => {
    ...
    state.defendant.attendance = 'لم يحضر';
    state.defendant.tabligh = '456';
    // وتُفرد بسطر خالٍ عن فقرة الافتتاح، فتسقط الفاصلة المعلَّقة في آخرها
    assert.match(
        M.composeMinutes(state),
        /^لدي أنا فلان بن فلان في المحكمة الرياض\n\nوقد تبلـــــــــغ الـمـدعى عليه عــــــــــــــبر الوسائل الإلكترونية بمهمة التبليغ رقم \(456\)، \n\n/
    );
});
...
test('buildPlaintiffEvidenceText: غياب المدعى عليه يُصرّح بلقب المدعي بدل الضمير', () => {
    ...
    const text = M.composeMinutes(state);
    assert.match(text, /\n\nوبسؤال المدعي عن بينته قرر قائلاً:/);
    assert.ok(!text.includes('وبسؤاله عن بينته'));
